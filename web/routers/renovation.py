...
from application.renovation_use_cases import RenovationUseCases
try:
    import openpyxl
...
renovation_uc = RenovationUseCases()
...
def get_renovacion_diagnostico():
    return renovation_uc.diagnostico(resolve_glpi)


@router.get("/renovacion")
def get_renovacion(modo_prueba: bool = True):
    return renovation_uc.analizar(
        modo_prueba=modo_prueba,
        glpi_factory=resolve_glpi,
        parse_glpi_full=_parse_glpi_computers_full,
        analizar_renovacion=_analizar_renovacion,
        datos_prueba=_DATOS_PRUEBA_RENOVACION,
    )
...
def post_renovacion_confirmar(payload: RenovacionConfirmarIn):
    return renovation_uc.confirmar(payload, resolve_glpi, _aplicar_par_renovacion)
